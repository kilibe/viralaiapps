...
async function loadChart(entityId, timeRange, metricType) {
    try {
        ...
        if (currentChart) {
            ...
            currentChart = new window.ViralityChart('virality-chart', metrics, {
                metricType,
                width: 850,
                height: 400
            });
...
function initializeDropdowns() {
    ...
    dropdowns.forEach((dropdown, index) => {
        ...
        if (!btn || !content) {
            console.error(`Dropdown ${index} missing button or content`);
            return;
        }
        
        console.log(`Setting up dropdown ${index}:`, btn.textContent);
